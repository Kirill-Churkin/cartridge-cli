...
func createBenchmarkSpace(tarantoolConnection *tarantool.Connection, sharding bool) error {
	// Creating space.
	...
	_, err = tarantoolConnection.Exec(tarantool.Call(formatCommand, format))
	if err != nil {
		return err
	}
	...
	// Creating bucket_id index if sharding used.
	if sharding {
		createIndexCommand := fmt.Sprintf("box.space.%s:create_index", benchSpaceName)
		_, err = tarantoolConnection.Exec(tarantool.Call(createIndexCommand, []interface{}{
		benchSpacePrimaryIndexName,
			map[string]interface{}{
				"parts":         []string{"bucket_id"},
				"if_not_exists": true,
			},
		}))
	}
