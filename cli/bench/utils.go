...
func isUsedSharding(ctx context.BenchCtx) bool {
	return ctx.Buckets > 0
}
...
func spacePreset(ctx context.BenchCtx, tarantoolConnection *tarantool.Connection) error {
	...
	return createBenchmarkSpace(tarantoolConnection, isUsedSharding(ctx))
}
...
func calculateBucket(key string, ctx context.BenchCtx) int {
	return 0
}

func getOnConnectionOperations(ctx context.BenchCtx) map[string]OnConnectionOperation {
	if isUsedSharding(ctx) {
		return map[string]OnConnectionOperation{
			"insert": shardingInsertOperationOnConnection,
			"select": shardingSelectOperationOnConnection,
			"update": shardingUpdateOperationOnConnection,
			"delete": shardingDeleteOperationOnConnection,
		}
	}
	return map[string]OnConnectionOperation{
		"insert": insertOperationOnConnection,
		"select": selectOperationOnConnection,
		"update": updateOperationOnConnection,
		"delete": deleteOperationOnConnection,
	}
}
