...
type Request struct {
	operation             func(Request) // insertOperation, selectOperation or updateOperation.
	ctx                   context.BenchCtx
	tarantoolConnection   *tarantool.Connection
	results               *Results
	getRandomTupleCommand *string // for selectOperation or updateOperation.
}
...
type RequestsSequence struct {
	...
	currentCounter int
	// findNewRequestsGeneratorMutex provides thread-safe search for new generator.
